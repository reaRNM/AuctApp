...
# 1. SEARCH
search_term = st.text_input("🔍 Search Library (Title, Brand, Model, UPC, ASIN)", "")

# 2. LOAD DATA (Added model)
query = """
...
with col_detail:
    ...
    if selected:
        ...
        if product_series is not None:
            ...
            if st.button("🗑️ Delete Product from Library"):
            ...
            st.error("Error loading product.")
            ...
            # Fetch history for this specific product
            hist_query = """
            ...
            if not history_df.empty:
                ...
                s1.metric("Lowest Sold", f"${low:,.2f}")
                s2.metric("Average", f"${avg:,.2f}")
                s3.metric("Highest Sold", f"${high:,.2f}")
            else:
                st.caption("No sales history recorded for this product yet.")    
